...
def get_xarray_dataset_from_file_on_s3(s3_bucket: str, s3_key: str,
                                       engine: str = None) -> Dataset:
    ...
    :type s3_key: str
    :param engine: Engine to be used when reading the file with xarray
    :type engine: str
    ...
    if engine:
        with xr.open_dataset(local_file_path, engine=engine) as ds_:
            ds = ds_.load()
    else:
